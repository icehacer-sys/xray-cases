// Generate Volume 3's full book as gpt-image-2 designed pages (same forensic case-file
// template as Could You Spot It / Volume 1 / Volume 2). Per case a portrait question page
// + answer page, compositing the real X-ray via image-edit. Resumable.
// `npx tsx src/vol3pages.ts <maxNewPages>` limits how many NEW pages to make (for testing).
...
function qPrompt(c: BookCase): string {
  return `${STYLE}\nLAYOUT (question page): top-left a small orange folder icon with "CASE FILE ${String(c.n).padStart(2, "0")}" in bold orange caps; top-right a thin cyan EKG line. ` +
...
for (const c of cases) {
  ...
  if (!existsSync(xrayPath)) { console.log(`! case ${c.n}: X-ray not generated yet, skipping`); continue; }
