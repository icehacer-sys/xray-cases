...
function userPrompt(cond: Condition, blind: BlindImageRead): string {
  ...
  return [
    ...
    `Clinical vignette supplied with the teaching question: ${cond.symptom}. This is context, not evidence that tests or treatment occurred.`,
    `Expected diagnosis: ${cond.diagnosis}`,
    `Compare the intended answer against the independent differential. Set singleAnswerSupported=false if another diagnosis is equally or better supported and no visible distinguishing feature resolves it. Do not invent histology or use the intended label as evidence. Judge the most likely answer from the visible pattern together with the supplied clinical vignette. A possible lower-ranked alternative is not by itself a failure, but an equally supported or better supported alternative still fails. Do not require exclusion of every conceivable diagnosis. Do not claim proof of an underlying cause or patient outcome.`,
