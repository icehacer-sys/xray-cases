...
globalThis.fetch = async (_url, init) => {
  ...
  if (calls === 1) assert.ok(!body.includes('SECRET_INTENDED_ANSWER') && !body.includes('SECRET_CLINICAL_VIGNETTE') && !body.includes('displaced head'), 'first call must be genuinely blind');
  else { assert.match(body, /SECRET_INTENDED_ANSWER/); assert.match(body, /BLIND_READING_FIXTURE/); assert.match(body, /SECRET_CLINICAL_VIGNETTE/); }
